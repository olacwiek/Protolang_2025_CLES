...
apes <- read_xlsx("final_decisions/apes.xlsx")
vocs <- read_xlsx("final_decisions/vocs.xlsx")
...
metadata_vocs <- read_xlsx("../Coders/all_metadata.xlsx", sheet = "Metadata_Vocalizations")
metadata_apes <- read_xlsx("../Coders/all_metadata.xlsx", sheet = "Metadata_Apes")
...
library(MASS)
library(effects)
apes$Final_decision <- factor(apes$Final_decision)
apes0001 <- dplyr::select(apes, Final_decision, `Right answer`, `previous work`)
apes0001 <- na.omit(apes0001)
m <- polr(Final_decision ~ `Right answer` + `previous work`, data = apes0001, Hess = TRUE)
summary(m)

# compare to null model
m0 <- polr(Final_decision ~ `Right answer`, data = apes0001, Hess = TRUE)
summary(m0)
anova(m, m0) # no significant difference


# binomial regression model
library(lme4)

apes0001$Final_decision_binary <- ifelse(apes0001$Final_decision==0, 0, 1)
apes0001$Final_decision_binary <- factor(apes0001$Final_decision_binary)

m1 <- glm(Final_decision_binary ~ `Right answer` + `previous work`,
    data = apes0001, family = "binomial") 
summary(m1)

m00 <- glm(Final_decision_binary ~ `Right answer`,
           data = apes0001, family = "binomial") 
...
f <- list.files("../Ape study participants answers/", pattern = "trials.csv", full.names = T, recursive = T)
