...
f_coder01 <- list.files("../data_annotated/", pattern = "^SH", full.names = T)
f_coder02 <- list.files("../data_annotated/", pattern = "^SK", full.names = T)
f_coder03 <- list.files("../data_annotated/", pattern = "^MP", full.names = T)
f_coder04 <- list.files("../data_annotated/", pattern = "^F", full.names = T) # coder F


# dataframe with coder (= crowdsourced participant), 
# annotator (= person who rated the crowdsourced 
# participant's answer), task number. 
# Each spreadsheet contains data from multiple 
# coders, but the rating of the answers was 
# conducted by one individual.
files <- tibble ( file = c(f_coder01, f_coder02, f_coder03, f_coder04))
